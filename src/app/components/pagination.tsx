...
function PaginationComponent() {
    ...
    return (
        <div className="flex h-8 gap-2 items-center rounded-lg">
            <NavigationButton title="go first" icon={<ArrowLeftToLine opacity={.5} />} onClick={goFirst} />
            <NavigationButton title="go previous" icon={<ArrowLeft opacity={.5} />} onClick={goPrevious} />
            <div className="h-full grid place-content-center aspect-square">
                <b title="current index"> {currentIndex} </b>
            </div>
            <NavigationButton title="go next" icon={<ArrowRight opacity={.5} />} onClick={goNext} />
            <NavigationButton title="go last" icon={<ArrowRightToLine opacity={.5} />} onClick={goLast} />
        </div>
    );
}
